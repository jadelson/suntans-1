...
void GetGrid(gridT **localgrid, int myproc, int numprocs, MPI_Comm comm)
{
  ...
  if(!TRIANGULATE) {
    ...
    //commented out by Gang. Got FPU_CW error when compling
  ...
  if((int)MPI_GetValue(DATAFILE,"CorrectVoronoi","ReadMainGrid",0)==1)
    CorrectVoronoi(maingrid);   
  
  if((int)MPI_GetValue(DATAFILE,"CorrectVoronoi","ReadMainGrid",0)==2)
    ChangeVoronoi(maingrid);
  ...
  if(VERBOSE>0) printf("Computing Graph...\n");
...
void GetDepth(gridT *grid, int myproc, int numprocs, MPI_Comm comm)
{
  ...
  //Added by Gang for getting depth directly
    Nd = MPI_GetSize(INPUTDEPTHFILE,"GetDepth",myproc);
    ifile = MPI_FOpen(INPUTDEPTHFILE,"r","GetDepth",myproc);
 
    for(n=0;n<grid->Nc;n++) {
      //grid->dv[n]=ReturnDepth(grid->xv[n],grid->yv[n]);
      temp = getfield(ifile,str);
      temp2 = getfield(ifile,str);
...
void ReadGrid(gridT **grid, int myproc, int numprocs, MPI_Comm comm) 
{
  ...
  FILE *ifile;
  REAL temp;
  int new_num_edges_send,new_num_edges_recv, *new_edge_send, *new_edge_recv, *remv_send,*remv_recv,both_own;
  ...
  //********************************************************************************
  //Added by Gang to get rid of edges owned by both two neighbouring processors
  for(neigh=0;neigh<(*grid)->Nneighs;neigh++) {
    new_num_edges_send=0;
    new_num_edges_recv=0;	
    new_edge_send=(int *)SunMalloc((*grid)->num_edges_send[neigh]*sizeof(int),"ReadGrid");	
    new_edge_recv=(int *)SunMalloc((*grid)->num_edges_recv[neigh]*sizeof(int),"ReadGrid");
    remv_send=(int *)SunMalloc((*grid)->num_edges_send[neigh]*sizeof(int),"ReadGrid");	
    remv_recv=(int *)SunMalloc((*grid)->num_edges_recv[neigh]*sizeof(int),"ReadGrid");
    ...
    for(n=0;n<(*grid)->num_edges_send[neigh];n++){
      ...
      for(nf=0;nf<(*grid)->num_edges_recv[neigh];nf++)
        if((*grid)->edge_send[neigh][n]==(*grid)->edge_recv[neigh][nf]){
          both_own=1;
          break;
        }	
        
      if(both_own){
        if(myproc<(*grid)->myneighs[neigh])
          remv_recv[nf]=1;
        else
          remv_send[n]=1;
      }
    ...
    for(n=0;n<(*grid)->num_edges_send[neigh];n++){
      if(!remv_send[n]){
      	new_edge_send[new_num_edges_send]= (*grid)->edge_send[neigh][n];
      	new_num_edges_send++;
      }	
    }
    ...
    for(n=0;n<(*grid)->num_edges_recv[neigh];n++){
      if(!remv_recv[n]){
      	new_edge_recv[new_num_edges_recv]= (*grid)->edge_recv[neigh][n];
      	new_num_edges_recv++;
      }	
    }
    
    for(n=0;n<new_num_edges_send;n++)
      (*grid)->edge_send[neigh][n]=new_edge_send[n];
    for(n=0;n<new_num_edges_recv;n++)
      (*grid)->edge_recv[neigh][n]=new_edge_recv[n];
    
    (*grid)->num_edges_send[neigh]=new_num_edges_send;
    (*grid)->num_edges_recv[neigh]=new_num_edges_recv;
  ...
  if(VERBOSE>2) printf("proc: %d, Reading %s...\n",myproc,VERTSPACEFILE);
  ifile = MPI_FOpen(VERTSPACEFILE,"r","ReadGrid",myproc);
  for(n=0;n<(*grid)->Nkmax;n++)
    (*grid)->dz[n]=getfield(ifile,str);
  fclose(ifile);
  printf("proc: %d, after Reading \n",myproc);
  ...
  //Added by Gang 12.07.05 to read in cell file
  (*grid)->cells = (int *)SunMalloc((*grid)->Nc*NFACES*sizeof(int),"ReadGrid");
  ...
  for(n=0;n<(*grid)->Nc;n++) {
    ...
    getfield(ifile,str);
    for(nf=0;nf<NFACES;nf++)
      (*grid)->cells[NFACES*n+nf]=(int)getfield(ifile,str);
    for(nf=0;nf<NFACES;nf++)
      getfield(ifile,str);
  }
  fclose(ifile);
  
  (*grid)->Np = MPI_GetSize(POINTSFILE,"GetGrid",myproc);
  // (x,y) coordinates of vertices
  (*grid)->xp = (REAL *)SunMalloc((*grid)->Np*sizeof(REAL),"InitMainGrid");
  (*grid)->yp = (REAL *)SunMalloc((*grid)->Np*sizeof(REAL),"InitMainGrid");
  ifile = MPI_FOpen(POINTSFILE,"r","ReadMainGrid",myproc);
...
static void VertGrid(gridT *maingrid, gridT **localgrid, MPI_Comm comm)
{
  ...
  if((*localgrid)->Nkmax>1) {
	...
	for(k=0;k<(*localgrid)->Nkmax;k++) {
	  ...
	  //Changed by Gang 08.24.05
	  //if((dmaxtest+0.001)>=(*localgrid)->dv[i]) 
	    break;
	}
      }
    for(i=0;i<maingrid->Nc;i++) 
...
REAL GetArea(REAL *xt, REAL *yt, int Nf)
{
  ...
  if(Nf==3) {
    b=sqrt((xt[1]-xt[0])*(xt[1]-xt[0])+
	   (yt[1]-yt[0])*(yt[1]-yt[0]));
    if(b<1e-6) return 0.0;
    r1=sqrt((xt[2]-xt[0])*(xt[2]-xt[0])+
	   (yt[2]-yt[0])*(yt[2]-yt[0]));
    r2=sqrt((xt[2]-xt[1])*(xt[2]-xt[1])+
	   (yt[2]-yt[1])*(yt[2]-yt[1]));
...
{
  int n,nf;	//,i,nc1,nc2;
  REAL xt[NFACES],yt[NFACES];//,area,xt2[NFACES],yt2[NFACES],subarea;
  REAL xc,yc,eps=1e-4;
  //REAL xv1, xv2, yv1, yv2, xc1, xc2, yc1, yc2, dg;
  REAL VoronoiRatio=MPI_GetValue(DATAFILE,"VoronoiRatio","CorrectVoronoi",0);
  REAL ang, MaxAngle;
  int jp[]={1,2,0}, jm[]={2,0,1};
  
  MaxAngle=VoronoiRatio;
  //If voronoi point is outside of the triangle. Use centroid
  for(n=0;n<grid->Nc;n++) {
    xc=0;
    yc=0;
    for(nf=0;nf<NFACES;nf++) {
      xt[nf]=grid->xp[grid->cells[n*NFACES+nf]];
      yt[nf]=grid->yp[grid->cells[n*NFACES+nf]];
      xc+=xt[nf];
      yc+=yt[nf];
    }
    xc/=NFACES;
    yc/=NFACES;
    for(nf=0;nf<NFACES;nf++) {
      ang=((xt[jp[nf]]-xt[nf])*(xt[jm[nf]]-xt[nf])+(yt[jp[nf]]-yt[nf])*(yt[jm[nf]]-yt[nf]))
            /(sqrt(pow(xt[jp[nf]]-xt[nf],2)+pow(yt[jp[nf]]-yt[nf],2))*sqrt(pow(xt[jm[nf]]-xt[nf],2)+pow(yt[jm[nf]]-yt[nf],2)));
      ang=acos(ang)*180/PI;
      if (ang>=MaxAngle){
        grid->xv[n]=xc;
        grid->yv[n]=yc;
