...
static void WriteAllProfileData(sediT *sedi, gridT *grid, physT *phys, propT *prop, MPI_Comm comm, int numprocs, int myproc);
 ...
 *
 */
void InterpData(sediT *sedi, gridT *grid, physT *phys, propT *prop, MPI_Comm comm, int numprocs, int myproc) {
  ...
  if(existProfs && !(prop->n%ntoutProfs)) 
    WriteAllProfileData(sedi,grid,phys,prop,comm,numprocs,myproc);
...
static void OpenDataFiles(int myproc) {
  ...
  if(strlen(ProfileVariables)>0) {
    ...
    if(ContainsCharacter(ProfileVariables,'C')) {
      MPI_GetFile(filename,DATAFILE,"SSCFile","OpenDataFiles",myproc);
      if(myproc==0) {
        sprintf(str,"%s.prof",filename);
        SediProfFID = fopen(str,"w");
      }
    }
//Kurt Nelson added for two individual outputs
    if(ContainsCharacter(ProfileVariables,'z')) {
      MPI_GetFile(filename,DATAFILE,"SSCZeroFile","OpenDataFiles",myproc);
      if(myproc==0) {
        sprintf(str,"%s.prof",filename);
        SediProfZeroFID = fopen(str,"w");
      }
    }
    if(ContainsCharacter(ProfileVariables,'o')) {
      MPI_GetFile(filename,DATAFILE,"SSCOneFile","OpenDataFiles",myproc);
      if(myproc==0) {
        sprintf(str,"%s.prof",filename);
        SediProfOneFID = fopen(str,"w");
      }
    }
/////////////////////////////////////////////////////////////////////////
...
//Kurt Nelson added
  if(ContainsCharacter(ProfileVariables,'z'))
    fclose(SediProfZeroFID);
  if(ContainsCharacter(ProfileVariables,'o'))
    fclose(SediProfOneFID);
/////////////////////////////////////////////////////////
  if(ContainsCharacter(ProfileVariables,'k'))
 ...
 * Usage: WriteAllProfileData(sedi,grid,phys,prop,comm,numprocs,myproc);
 * ----------------------------------------------------------------
...
static void WriteAllProfileData(sediT *sedi, gridT *grid, physT *phys, propT *prop, MPI_Comm comm, int numprocs, int myproc) {
  ...
  if(ContainsCharacter(ProfileVariables,'C'))
    Write3DData(sedi->sdtot,grid->Nk,NkmaxProfs,merge_tmp,merge_tmp2,SediProfFID,comm,numprocs,myproc);
//Kurt Nelson added
  if(ContainsCharacter(ProfileVariables,'z'))
    Write3DData(sedi->scc0,grid->Nk,NkmaxProfs,merge_tmp,merge_tmp2,SediProfZeroFID,comm,numprocs,myproc);
  if(ContainsCharacter(ProfileVariables,'o'))
    Write3DData(sedi->scc1,grid->Nk,NkmaxProfs,merge_tmp,merge_tmp2,SediProfOneFID,comm,numprocs,myproc);
///////////////////////////////////////////////////////////////////////////////////
}
