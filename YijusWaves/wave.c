...
static REAL InterpCgToFace(int m, int n, int j, waveT *wave, gridT *grid);
static REAL semivariogram(REAL Cov0, REAL Dmax, REAL D);
static void OperatorN(int m, int n, REAL *x, REAL *y, gridT *grid, waveT *wave, wpropT *wprop, propT *prop);
static void ObtainWaveVelocity(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc);
static void ObtainEdgeUwField(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc);
static void CenterRadiationStress(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc);
static void EdgeRadiationStress(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc);
...
static void UpdateActionDensitySinkSource(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc, int numprocs);
static void ImplicitUpdateGeographic(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc, int numprocs);
static void ExplicitUpdateGeographic(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc, int numprocs);
static REAL InterpWindShearToFace(int j, physT *phys, waveT *wave, gridT *grid);
static void ObtainTotalEnergy(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc);
static void ObtainEdgeCgField(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc);
static void UpdateActionDensitySpectral(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc);
static void ObtainCenterCgField(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc);
static void ObtainCenterCsCtField(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc);
static void ObtainKField(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop); 
static void BiCGSolveN(int m0, int n0, gridT *grid, waveT *wave, wpropT *wprop, propT *prop, int myproc, int numprocs, MPI_Comm comm);
...
void InitializeWaveProperties(wpropT **wprop, propT *prop, int myproc)
{
  ...
  *wprop = (wpropT *)SunMalloc(sizeof(wpropT), "InitializeWaveProperties");
  ...
  (*wprop)->Mw = MPI_GetValue(WAVEFILE,"Mw","InitializeWaveProperties",myproc); 
  (*wprop)->Nw = MPI_GetValue(WAVEFILE,"Nw","InitializeWaveProperties",myproc);
  (*wprop)->wnstep = MPI_GetValue(WAVEFILE,"wnstep","InitializeWaveProperties",myproc);
  (*wprop)->sgmin = MPI_GetValue(WAVEFILE,"sgmin","InitializeWaveProperties",myproc);
  (*wprop)->sgmax = MPI_GetValue(WAVEFILE,"sgmax","InitializeWaveProperties",myproc);
  (*wprop)->wind_dt = MPI_GetValue(WAVEFILE,"wind_dt","InitializeWaveProperties",myproc);
  (*wprop)->implicit_whitecap = MPI_GetValue(WAVEFILE,"implicit_whitecap","InitializeWaveProperties",myproc);
  (*wprop)->implicit_advection = MPI_GetValue(WAVEFILE,"implicit_advection","InitializeWaveProperties",myproc);
  (*wprop)->wind_forcing = MPI_GetValue(WAVEFILE,"wind_forcing","InitializeWaveProperties",myproc);
  (*wprop)->nstation = MPI_GetValue(WAVEFILE,"nstation","InitializeWaveProperties",myproc);
  (*wprop)->tail_opt = MPI_GetValue(WAVEFILE,"tail_opt","InitializeWaveProperties",myproc);
  (*wprop)->tail_pow = MPI_GetValue(WAVEFILE,"tail_pow","InitializeWaveProperties",myproc);
  (*wprop)->wind_shear = MPI_GetValue(WAVEFILE,"wind_shear","InitializeWaveProperties",myproc);
  (*wprop)->rad_stress = MPI_GetValue(WAVEFILE,"rad_stress","InitializeWaveProperties",myproc);
  (*wprop)->form_drag = MPI_GetValue(WAVEFILE,"form_drag","InitializeWaveProperties",myproc);
  (*wprop)->btm_mud = MPI_GetValue(WAVEFILE,"btm_mud","InitializeWaveProperties",myproc);
  (*wprop)->btm_sedi_erosion = MPI_GetValue(WAVEFILE,"btm_sedi_erosion","InitializeWaveProperties",myproc);
  (*wprop)->depth_fw_cutoff = MPI_GetValue(WAVEFILE,"depth_fw_cutoff","InitializeWaveProperties",myproc);
  (*wprop)->fw_drag = MPI_GetValue(WAVEFILE,"fw_drag","InitializeWaveProperties",myproc);
  (*wprop)->btm_conc = MPI_GetValue(WAVEFILE,"btm_conc","InitializeWaveProperties",myproc);
  (*wprop)->btm_vis = MPI_GetValue(WAVEFILE,"btm_vis","InitializeWaveProperties",myproc);
  (*wprop)->btm_mud_thickness = MPI_GetValue(WAVEFILE,"btm_mud_thickness","InitializeWaveProperties",myproc);
  (*wprop)->depth_brk_cutoff = MPI_GetValue(WAVEFILE,"depth_brk_cutoff","InitializeWaveProperties",myproc);
  (*wprop)->depth_brk_indx = MPI_GetValue(WAVEFILE,"depth_brk_indx","InitializeWaveProperties",myproc);
  (*wprop)->NLtriad = MPI_GetValue(WAVEFILE,"NLtriad","InitializeWaveProperties",myproc);
  (*wprop)->NLquad = MPI_GetValue(WAVEFILE,"NLquad","InitializeWaveProperties",myproc);
  (*wprop)->BRKdepth = MPI_GetValue(WAVEFILE,"BRKdepth","InitializeWaveProperties",myproc);
  ...
  for (i=1; i<= Ns; i++){
    sprintf(xstr, "xw.%d", i);
    (*wprop)->xw[i-1] = MPI_GetValue(WAVEFILE,xstr,"InitializeWaveProperties",myproc);
    sprintf(ystr, "yw.%d", i);
    (*wprop)->yw[i-1] = MPI_GetValue(WAVEFILE,ystr,"InitializeWaveProperties",myproc);
...
void InputWind(int station, propT *prop, waveT *wave, wpropT *wprop, int myproc, int numprocs)
{
  ...
  Noffset = 0;//floor((prop->nstart*prop->dt)/wprop->wind_dt); //tsungwei test0703
  sprintf(fd, "WindFile.%d", station+1);
  MPI_GetFile(filename, WAVEFILE, fd, "InputWind",myproc);
...
void SourceByWind(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc){
...
void SinkByWhitecapping(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc){
...
void SinkByWhitecapping_implicit(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc){
...
void SinkByMud_new(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, sediT *sedi, spropT *sprop, MPI_Comm comm, int myproc){
  ...
  if (prop->sedi)
    conc = sedi->Dl[0]/sprop->spwght*0.000001;
  else
    conc = wprop->btm_conc;
...
//void SinkByMud(int option, gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, sediT *sedi, spropT *sprop, MPI_Comm comm, int myproc){
void SinkByMud(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, sediT *sedi, spropT *sprop, MPI_Comm comm, int myproc){  
  int i, m, n, l, Mw=wprop->Mw,Nw=wprop->Nw, Nc=grid->Nc, Nl = sprop->NL;
  REAL dpth, kd, St_thickness, dm, gamma, cg, Dw, conc, fac, Cb, kwd, fw;
  ...
  for (m=0; m < Mw; m++){
	...
	kd = wave->kw[m][i]*dpth;
	if(wave->sg[m] > 0){
	  St_thickness = sqrt(2*sprop->vis/wave->sg[m]);
	  dm = 0;
	  //	  for (l = 0; l< Nl-1; l++)
	  //	    dm += sedi->hl[l];
	  //dm = 0.02; 
...
void SinkByDrag(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc){
...
void SinkByBreaking(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc){
...
void SourceByTriad(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc){
...
void SourceByQuad(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc){
...
void ObtainTotalEnergy(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc){
...
void ObtainMeanKSG(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc){
  ...
  for(i=0; i< Nc; i++){
	...
	if (dpth > 0){
      ...
      //The wave velocities obtained by this (the following 2 lines) seem to be too small for sediment erosion
...
void ObtainKField(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop) 
{
  int i,m;
  REAL dpth, G, tmp, cph;
...
void ObtainEdgeKField(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop) 
{
  int j, m, Ne = grid->Ne, Mw = wprop->Mw;
  REAL dpth, G, tmp, cph;
...
void ObtainCenterCgField(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc)
{
  int i, m, n, Mw=wprop->Mw,Nw=wprop->Nw, Nc=grid->Nc;
...
void ObtainEdgeCgField(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc)
{
  ...
  for (m=0; m < Mw; m++){
	...
	if(nc1 == -1){
	  ...
	  wave->cg[m][n][j] = InterpCgToFace(m, n, j, wave, grid);
	}
      }
...
void ObtainEdgeWaveProp(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc)
{
...
void ObtainCenterCsCtField(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc)
{
...
MPI_Comm comm, int myproc, int numprocs){
  ...
  ObtainKField(grid, phys, wave, prop, wprop);
  ObtainEdgeKField(grid, phys, wave, prop, wprop); 
  ObtainCenterCgField(grid, phys, wave, prop, wprop, comm, myproc);
  ObtainEdgeCgField(grid, phys, wave, prop, wprop, comm, myproc);
  ObtainCenterCsCtField(grid, phys, wave, prop, wprop, comm, myproc);
  if (wprop->wind_forcing)
    SourceByWind(grid, phys, wave, prop, wprop, comm, myproc);
  
  if (wprop->form_drag)
    SinkByDrag(grid, phys, wave, prop, wprop, comm, myproc);
  if (wprop->btm_mud)
    SinkByMud_new(grid, phys, wave, prop, wprop, sedi, sprop, comm, myproc);
  if (wprop->NLquad)
    SourceByQuad(grid, phys, wave, prop, wprop, comm, myproc);
  if (wprop->NLtriad)
    SourceByTriad(grid, phys, wave, prop, wprop, comm, myproc);
  if (wprop->BRKdepth)
    SinkByBreaking(grid, phys, wave, prop, wprop, comm, myproc);  
  if (wprop->implicit_whitecap)
    SinkByWhitecapping_implicit(grid, phys,wave, prop, wprop, comm, myproc);
  else{
    SinkByWhitecapping(grid, phys,wave, prop, wprop, comm, myproc);
    UpdateActionDensitySinkSource(grid, phys, wave, prop, wprop, comm, myproc, numprocs);
  }

  if (wprop->implicit_advection == 1)
    ImplicitUpdateGeographic(grid, phys, wave, prop, wprop, comm, myproc, numprocs);
  else
    ExplicitUpdateGeographic(grid, phys, wave, prop, wprop, comm, myproc, numprocs); 

  UpdateActionDensitySpectral(grid, phys, wave, prop, wprop, comm, myproc);
  ObtainTotalEnergy(grid, phys, wave, prop,wprop,comm, myproc);
  ObtainMeanKSG(grid, phys, wave, prop, wprop, comm, myproc);
  ObtainWaveVelocity(grid, phys, wave, prop, wprop, comm, myproc);
  ObtainEdgeWaveProp(grid, phys, wave, prop, wprop, comm, myproc);    
  ObtainEdgeUwField(grid, phys, wave, prop, wprop, comm, myproc); 
  CenterRadiationStress(grid, phys, wave, prop, wprop, comm, myproc);
  EdgeRadiationStress(grid, phys, wave, prop, wprop, comm, myproc);
  
}

void UpdateActionDensitySpectral(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc){
...
void UpdateActionDensitySinkSource(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc, int numprocs)
{
...
void ExplicitUpdateGeographic(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc, int numprocs)
{
...
void ImplicitUpdateGeographic(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc, int numprocs)
{  
  int i, iptr, m, nf, ne, n, nc1, nc2, Nc = grid->Nc, Mw=wprop->Mw, Nw=wprop->Nw, normal, flag = 0;
  REAL dt = prop->dt, source, Ac, df, dg;
  ...
  for (m=0; m<Mw; m++){
    for (n=0; n<Nw; n++){	
      BiCGSolveN(m, n, grid, wave, wprop, prop, myproc, numprocs, comm);
    }
  }
}
...
void WindField(propT *prop, gridT *grid, physT *phys, waveT *wave, wpropT *wprop, int myproc, int numprocs)
{
  ...
  if (wprop->wind_shear == 1){
    for (j=0; j< Ne; j++)            
      phys->tau_T[j] = InterpWindShearToFace(j, phys, wave, grid);
  }
}

void WaveTurbMixing(gridT *grid, physT *phys, waveT *wave, sediT *sedi, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc){
  ...
  for(i = 0; i < Nc; i++){
    ...
    if (wave->ab[i] > 0.02){
      ...
      lz0 = log10(sedi->kb[i]/30*wave->sgmean[i]/wave->ub[i]);
      z = 0;
...
static void BiCGSolveN(int m0, int n0, gridT *grid, waveT *wave, wpropT *wprop, propT *prop, int myproc, int numprocs, MPI_Comm comm)
{
  ...
  OperatorN(m0, n0, x, z, grid, wave, wprop, prop);
  ...
  if (eps > epsW){
	...
	p[i] = r[i]+beta*(p[i]-omg*v[i]); 
      ...
      OperatorN(m0, n0, p, v, grid, wave, wprop, prop);
	...
	break;
      ...
      OperatorN(m0, n0, s, t, grid, wave, wprop, prop);
...
void CenterRadiationStress(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc)
{
  int i, k;
...
void EdgeRadiationStress(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc)
{
  int j, k, flag, jptr;
...
void ObtainWaveVelocity(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc)
{
  int i, k;
...
void ObtainEdgeUwField(gridT *grid, physT *phys, waveT *wave, propT *prop, wpropT *wprop, MPI_Comm comm, int myproc)
{
