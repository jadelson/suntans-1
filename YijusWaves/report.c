...
void ParseFlags(int argc, char *argv[], int myproc)
{
  ...
  sprintf(SEDIFILE,"%s/%s",DATADIR,DEFAULTSEDIFILE);
  sprintf(WAVEFILE,"%s/%s",DATADIR,DEFAULTWAVEFILE);
  //sprintf(WINDFILE,"%s/%s",DATADIR,DEFAULTWINDFILE);
  ...
  if(argc>1) {
	...
	if(argv[i][1]=='t' && strlen(argv[i])==2)
	  ...
	  if(!strcmp(str,"datadir")) {
	    ...
	    GetValue(SEDIFILE,str,&status);
	    //GetValue(WINDFILE,str,&status);
	    GetValue(WAVEFILE,str,&status);
