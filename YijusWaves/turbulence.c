 ...
 *
 */
void my25(gridT *grid, physT *phys, sediT *sedi, propT *prop, REAL **q, REAL **l, REAL **Cn_q, REAL **Cn_l, REAL **nuT, REAL **kappaT, MPI_Comm comm, int myproc) {
  int i, ib, j, iptr, jptr, k, nf, nc1, nc2, ne;
  REAL thetaQ=1, CdAvgT, CdAvgB, *dudz, *dvdz, *drdz, z, *N, *Gh, tauAvgT;
  REAL A1, A2, B1, B2, C1, E1, E2, E3, Sq, Sm, Sh, tmp;
  ...
  for(i=0;i<grid->Nc;i++) {
    ...
    */
    ...
    for(k=grid->ctop[i];k<grid->Nk[i];k++) {
      ...
      //sedi->G[i][k] = 2.0*pow(q[i][k], 3)/B1/(l[i][k]+SMALL);
      //sedi->G[i][k] = 2.0*sqrt(pow(0.5*(dudz[k]+dudz[k+1]),2)+pow(0.5*(dvdz[k]+dvdz[k+1]),2));
      ...
      //      phys->wtmp[i][k] += wave->sgmean[i]*pow(wave->ub[i], 2)*pow(wave->fphi[i]*wave->fz[i][k], 3); 
